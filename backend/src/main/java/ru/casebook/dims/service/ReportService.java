...
public class ReportService {
    ...
    public ReportPreviewResponse preview(UUID caseId) {
        return preview(caseId, "FULL");
    }

    public ReportPreviewResponse preview(UUID caseId, String template) {
        ...
        return new ReportPreviewResponse(render(caseFile, openLabs, template), openLabs, openLabs ? "Есть незавершенные лабораторные запросы" : null);
    }

    @Transactional
    public ReportFile approve(UserAccount actor, UUID caseId, boolean force) {
        return approve(actor, caseId, force, "FULL");
    }

    @Transactional
    public ReportFile approve(UserAccount actor, UUID caseId, boolean force, String template) {
        ...
        String content = render(caseFile, openLabs, template);
    ...
    private String render(CaseFile caseFile, boolean openLabs) {
        return render(caseFile, openLabs, "FULL");
    }

    private String render(CaseFile caseFile, boolean openLabs, String template) {
        ...
        if ("SUMMARY".equalsIgnoreCase(template)) {
            out.append("Краткая сводка: улик — ").append(evidence.findByCaseFileId(caseId).size())
                    .append(", экспертиз — ").append(labs.findByCaseFileId(caseId).size())
                    .append(", связей — ").append(edges.findByCaseFileId(caseId).size()).append('\n');
            if (openLabs) out.append("Данные ожидаются по незавершенным запросам.\n");
            return out.toString();
        }
        out.append("Улики:\n");
